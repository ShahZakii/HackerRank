# HackerRank Python Question.

# Question 8:
# Given the names and grades for each student in a class of  students, store them in a nested list and print the name(s) of any 
# student(s) having the second lowest grade.

# Note: If there are multiple students with the second lowest grade, order their names alphabetically and print each 
# name on a new line.

def sorted_student(name_score):
    scores = []
    for student in name_score:
        scores.append(student[1])
    sorted_score = sorted(set(scores))
    second_lowest = sorted_score[1]

    new_second_lowest = []
    for student in name_score:
        if student[1] == second_lowest:
            new_second_lowest.append(student[0])
    
    sorted_names = sorted(new_second_lowest)
    for name in sorted_names:
        print(name)
        

if __name__ == '__main__':
    name_score = []
    for _ in range(int(input())):
        name = input()
        score = float(input())
        name_score.append([name,score])

    sorted_student(name_score)